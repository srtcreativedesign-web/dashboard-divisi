...
export default function DailyReportPage() {
  ...
  const [selectedDetail, setSelectedDetail] = useState<DailyRecord | null>(null);
  ...
  const handleSaveDailyFromModal = (reportData: Omit<DailyRecord, 'id' | 'status' | 'submittedAt'>) => {
    addDailyReport(
      reportData,
      user?.name ?? reportData.updatedBy,
      user?.role ?? 'PIC'
    );
    showToast(`✅ Laporan Omset ${reportData.divisionName}${reportData.outletName ? ` (${reportData.outletName})` : ''} berhasil disubmit! Menunggu ACC Manager.`);
  };

  const handleOpenDetail = (record: DailyRecord) => {
  ...
  return (
    <div className="space-y-6 relative min-h-full" data-testid="daily-report-page">
          ...
          <div className="mt-6 overflow-x-auto rounded-card-lg border border-line/40">
            <table className="w-full text-left text-sm">
              <thead className="bg-surface text-xs font-semibold uppercase tracking-wider text-slate-500">
                <tr>
                  <th className="px-4 py-3.5">Tanggal & Outlet / Divisi</th>
                  <th className="px-4 py-3.5 text-right">Omset & Rincian Kasir (Rp)</th>
                  <th className="px-4 py-3.5 text-right">Target & Capaian (Rp)</th>
                    ...
                    <tr key={item.id} className="hover:bg-surface/50 transition-colors" data-testid={`daily-row-${item.id}`}>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-bold text-navy">{item.division} - {item.divisionName}</span>
                          {item.shift && (
                            <span className="text-[10px] rounded-pill bg-slate-100 text-slate-600 px-1.5 py-0.5 border border-slate-200">
                              {item.shift.includes('Pagi') ? 'Pagi' : item.shift.includes('Siang') ? 'Siang' : '24 Jam'}
                            </span>
                          )}
                        </div>
                        {item.outletName ? (
                          ...
                          <p className="text-[11px] text-slate-400 italic">Konsolidasi Seluruh Outlet</p>
                        )}
                        <p className="text-[11px] font-mono text-slate-400 mt-0.5">{item.date}</p>
                      </td>
                      <td className="px-4 py-3.5 text-right font-mono">
                        <p className="font-bold text-navy text-sm">
                          Rp {item.revenue.toLocaleString('id-ID')}
                        </p>
                        {(item.cashAmount || item.edcAmount || item.qrisAmount) ? (
                        ...
                        {item.attachmentName && (
                          <span className="inline-flex items-center gap-0.5 text-[10px] text-emerald-700 font-sans mt-0.5" title={item.attachmentName}>
                            <Paperclip className="w-2.5 h-2.5" /> Slip EDC
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3.5 text-right font-mono">
                        ...
                        {item.target > 0 && (
                          <p className={`text-[11px] font-bold ${
                            item.revenue >= item.target
                              ? 'text-success'
                              : item.revenue >= item.target * 0.8
                              ? 'text-info'
                              : 'text-warning'
                          }`}>
                            {Math.round((item.revenue / item.target) * 100)}% Capaian
                          </p>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-center">
                        <div className="flex items-center justify-center gap-1.5 flex-wrap">
                          <button
                            ...
                            title="Lihat Rincian Laporan Lengkap"
                            data-testid={`btn-detail-daily-${item.id}`}
                          >
      ...
      {/* Modal Input Omset Harian Baru (Terhubung ke 58 Outlet Sobat API & 7 Divisi) */}
      <DailyReportInputModal
        ...
        userName={user?.name}
        onSubmit={handleSaveDailyFromModal}
      />

      {/* Modal Detail Laporan (Inspeksi Menyeluruh) */}
